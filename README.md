...
| `pnpm release`         | сборка + `changeset publish`; запускается из CI, не руками   |
...
`CHANGELOG.md` и удалением съеденных `.md`. Мерж этого PR — и есть релиз: тот же workflow
запускает `pnpm release`, публикует пакет в npm, ставит git-тег и создаёт GitHub Release.
